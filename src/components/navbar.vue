<template>
  <header
    ...
    :style="{ background: `rgba(207,125,75, ${bgOpacity})` }"
    ...
    <div class="nav-main">
      <div class="nav-inner">
        <div class="logo">
          ...
          <slot name="logo">
            <div class="logo-placeholder">
              <img :src="logo_pelgone" alt="Logo Pelgone" class="logo-img" />
            </div>
          </slot>
        </div>
        ...
        <div class="nav-actions">
          <button class="cart">
            <a href="https://www.instagram.com/software.e_one/" style="text-decoration: none; color: black">
...
<script setup>
import logo_pelgone from '../assets/pelgone_logo.png';
...
const props = defineProps({
  ...
  bgFadeRange: { type: Number, default: 120 } // jarak scroll (px) untuk mencapai opacity penuh
});
...
function updateBgOpacity(currentY) {
  // hitung opacity berdasarkan range, clamped 0..1
  const op = clamp(currentY / props.bgFadeRange, 0, 1);
  bgOpacity.value = Number(op.toFixed(3)); // sedikit pembulatan
}
...
function processScroll() {
  ...
  // update background opacity (muncul bertahap)
  updateBgOpacity(currentY);

  // jika di paling atas -> selalu tampil (tetap kontrol visibilitas seperti sebelumnya)
...
.navbar-wrapper {
  ...
  width: 100%;
  z-index: 9999;
  /* background di-set inline dari binding :style agar bisa berubah bertahap */
...
.topbar .left {
  font-family: 'franklin gotic', sans-serif;
  display: flex;
  align-items: center;
  gap: 8px;
  font-weight: 700;
...
@media (max-width: 880px) {
  .nav-links {
    display: none;
  } /* hide big links on small screen (you can replace with hamburger) */
  .topbar .center {
    display: none;
  }
}
